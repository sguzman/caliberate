...
use crate::backend;
use crate::query::{BookMetadataFilterField, BookMetadataFilterMode, BookQuery, BookSortField};
...
fn query_parts(query: &BookQuery) -> CoreResult<(Vec<&'static str>, Vec<String>, Vec<Value>)> {
            ...
            BookMetadataFilterField::Ratings => {
                let rating = filter.value.parse::<i64>().map_err(|_| {
                    CoreError::ConfigValidate(format!(
                        "rating metadata filter must be an integer from 0 to 10: {}",
                        filter.value
                    ))
                })?;
                ...
                (
                    "EXISTS (SELECT 1 FROM books_ratings_link mf_brl JOIN ratings mf_r ON mf_r.id = mf_brl.rating WHERE mf_brl.book = b.id AND mf_r.rating = ?)",
                    Value::from(rating),
                )
            }
        };
        let predicate = match filter.mode {
            BookMetadataFilterMode::Include => predicate.to_string(),
            BookMetadataFilterMode::Exclude => predicate.replacen("EXISTS", "NOT EXISTS", 1),
        };
...
impl Database {
    ...
    fn apply_schema(&self) -> CoreResult<()> {
        self.conn
            .execute_batch(
                ...
                CREATE INDEX IF NOT EXISTS idx_source_books_source_id ON source_books(source_id);
                CREATE INDEX IF NOT EXISTS idx_source_books_book_id ON source_books(book_id);
        ...
        self.ensure_columns(
            ...
            &[
                (
                    "book_format_id",
                    "ALTER TABLE assets ADD COLUMN book_format_id INTEGER",
                ),
                (
                    "source_id",
                    "ALTER TABLE assets ADD COLUMN source_id INTEGER",
                ),
            ],
            "assets",
        )?;
        self.backfill_canonical_formats()?;
    ...
    fn backfill_canonical_formats(&self) -> CoreResult<()> {
        self.conn
            .execute(
    ...
    fn ensure_no_autoincrement(
        &self,
        table: &str,
        label: &str,
        rebuild: fn(&Database) -> CoreResult<()>,
    ) -> CoreResult<()> {
        if self.table_sql_contains(table, "AUTOINCREMENT")? {
            rebuild(self)?;
            info!(
                component = "db",
                table, "rebuilt table without AUTOINCREMENT"
            );
        }
        if self.table_sql_contains(table, "AUTOINCREMENT")? {
            return Err(CoreError::ConfigValidate(format!(
                "{label} still contains AUTOINCREMENT"
            )));
        }
        Ok(())
    }
    ...
    fn rebuild_table(
        ...
        let create_sql = create_sql.replace("{table}", &table_new);
        let column_list = columns.join(", ");
        let sql = format!(
            "BEGIN;
             {create_sql};
             INSERT INTO {table_new} ({column_list})
                SELECT {column_list} FROM {table};
             DROP TABLE {table};
             ALTER TABLE {table_new} RENAME TO {table};
             COMMIT;"
        );
        self.rebuild_with_foreign_keys_off(&sql, label)
    }
    ...
    fn ensure_calibre_primary_keys(&self) -> CoreResult<()> {
        self.ensure_no_autoincrement("authors", "authors table", Self::rebuild_authors_table)?;
        self.ensure_no_autoincrement("tags", "tags table", Self::rebuild_tags_table)?;
        self.ensure_no_autoincrement("series", "series table", Self::rebuild_series_table)?;
        self.ensure_no_autoincrement(
            "publishers",
            "publishers table",
            Self::rebuild_publishers_table,
        )?;
        self.ensure_no_autoincrement("ratings", "ratings table", Self::rebuild_ratings_table)?;
        self.ensure_no_autoincrement(
            "languages",
            "languages table",
            Self::rebuild_languages_table,
        ...
        self.ensure_no_autoincrement("comments", "comments table", Self::rebuild_comments_table)?;
        self.ensure_no_autoincrement(
            "books_authors_link",
            "books_authors_link table",
            Self::rebuild_books_authors_link_table,
        )?;
        self.ensure_no_autoincrement(
            "books_tags_link",
            "books_tags_link table",
            Self::rebuild_books_tags_link_table,
        )?;
        self.ensure_no_autoincrement(
            "books_series_link",
            "books_series_link table",
            Self::rebuild_books_series_link_table,
        )?;
        self.ensure_no_autoincrement(
            "books_publishers_link",
            "books_publishers_link table",
            Self::rebuild_books_publishers_link_table,
        )?;
        self.ensure_no_autoincrement(
            "books_ratings_link",
            "books_ratings_link table",
            Self::rebuild_books_ratings_link_table,
        )?;
        self.ensure_no_autoincrement(
            "books_languages_link",
            "books_languages_link table",
            Self::rebuild_books_languages_link_table,
        )?;
        self.ensure_no_autoincrement(
            "books_plugin_data",
            "books_plugin_data table",
            Self::rebuild_books_plugin_data_table,
        )?;
        self.ensure_no_autoincrement(
            "conversion_options",
            "conversion_options table",
            Self::rebuild_conversion_options_table,
        )?;
        self.ensure_no_autoincrement(
            "custom_columns",
            "custom_columns table",
            Self::rebuild_custom_columns_table,
        )?;
        self.ensure_no_autoincrement("data", "data table", Self::rebuild_data_table)?;
        self.ensure_no_autoincrement("feeds", "feeds table", Self::rebuild_feeds_table)?;
        self.ensure_no_autoincrement(
            "library_id",
            "library_id table",
            Self::rebuild_library_id_table,
        )?;
        self.ensure_no_autoincrement(
            "metadata_dirtied",
            "metadata_dirtied table",
            Self::rebuild_metadata_dirtied_table,
        )?;
        self.ensure_no_autoincrement(
            "annotations_dirtied",
            "annotations_dirtied table",
            Self::rebuild_annotations_dirtied_table,
        )?;
        self.ensure_no_autoincrement(
            "preferences",
            "preferences table",
            Self::rebuild_preferences_table,
        )?;
        self.ensure_no_autoincrement(
            "last_read_positions",
    ...
    fn rebuild_books_authors_link_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "books_authors_link",
            "books_authors_link table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_books_tags_link_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "books_tags_link",
            "books_tags_link table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_books_series_link_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "books_series_link",
            "books_series_link table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_books_publishers_link_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "books_publishers_link",
            "books_publishers_link table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_books_ratings_link_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "books_ratings_link",
            "books_ratings_link table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_books_languages_link_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "books_languages_link",
            "books_languages_link table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_books_plugin_data_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "books_plugin_data",
            "books_plugin_data table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_conversion_options_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "conversion_options",
            "conversion_options table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_custom_columns_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "custom_columns",
            "custom_columns table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_feeds_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "feeds",
            "feeds table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_library_id_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "library_id",
            "library_id table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_metadata_dirtied_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "metadata_dirtied",
            "metadata_dirtied table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_annotations_dirtied_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "annotations_dirtied",
            "annotations_dirtied table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_preferences_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "preferences",
            "preferences table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_annotations_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "annotations",
            "annotations table",
            "CREATE TABLE {table} (
    ...
    fn rebuild_notes_table(&self) -> CoreResult<()> {
        self.rebuild_table(
            "notes",
            "notes table",
            "CREATE TABLE {table} (
    ...
    pub fn add_book(
        ...
        if !format.is_empty() {
            self.upsert_book_format(book_id, format, None)?;
        }
        Ok(book_id)
    }
    ...
    pub fn list_assets(&self) -> CoreResult<Vec<AssetRow>> {
        let mut stmt = self
            .conn
            .prepare(
                "SELECT id, book_id, storage_mode, stored_path, source_path, size_bytes, stored_size_bytes, checksum, is_compressed, created_at, book_format_id, source_id
    ...
    pub fn delete_book_with_assets(&mut self, book_id: i64) -> CoreResult<DeleteSummary> {
        ...
        tx.execute(
            "DELETE FROM source_books WHERE book_id = ?1",
            params![book_id],
        )
        .map_err(|err| sqlite_error("delete book source mappings", err))?;
        tx.execute(
            "DELETE FROM book_formats WHERE book_id = ?1",
            params![book_id],
        )
        .map_err(|err| sqlite_error("delete book formats", err))?;
        let book_deleted = tx
