# 0010.1 — Make the library shell panes resizable, collapsible, and center-safe

## Why

Runtime testing with real books after task `0010` exposed a severe usability defect in the main library shell: the Details pane can consume most of the window and the user cannot conveniently collapse it or recover enough space for the library browser.
...
This is an interstitial usability fix discovered while manually accepting task `0010`. It deliberately uses patch task ID `0010.1` so the already-assigned `0011-library-query-sort-parity` task remains intact and retains its identity.
...
- implement task `0011-library-query-sort-parity`;
...
Task `0011-library-query-sort-parity` remains the next normal numbered task after this usability fix.
...
Write `docs/work/reports/0010.1.md` with:
...
- `codex/0010.1-gui-pane-layout-ergonomics`
